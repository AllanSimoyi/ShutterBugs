...
import { UploadState, uploadToCloudinary, useCloudinary } from "remix-chakra-reusables";
...
export function useUploadImages (props: Props) {
  ...
  const [imageUploads, setImageUploads] = useState<ImageUploadState[]>(imageIds.map(imageId => ({
    imageId,
  ...
  const uploadImages = useCallback(async (files: File[]) => {
    try {
      ...
      const tooLarge = files.some(file => file.size > ImageUploadSizeLimit.Value);
      if (tooLarge) {
        throw new Error(`Please ensure you upload images less than ${ImageUploadSizeLimit.Caption}`);
      }
      ...
      const results = await Promise.all(
        files.map((file) => {
          return uploadToCloudinary(file, CLOUDINARY_CLOUD_NAME, CLOUDINARY_UPLOAD_RESET);
        })
      );
